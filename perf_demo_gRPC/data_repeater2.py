...
def getDataFromApi(count):
    with grpc.insecure_channel(f"localhost:{config.DATA_REPEATER_PORT1}") as channel:
        stub = data_service_pb2_grpc.DataServiceStub(channel)
        request = data_service_pb2.DataRequest(count=count)
        print("Retrieved data from data repeater 1")
...
class DataServiceServicer(data_service_pb2_grpc.DataServiceServicer):
    def GetData(self, request, context):
        ...
        print("Returning data from data repeater 2")
...
def serve():
    ...
    server.add_insecure_port(f"[::]:{config.DATA_REPEATER_PORT2}")
    print(f"Running server on port: {config.DATA_REPEATER_PORT2}")
    server.start()
